...
2. Внести валидные значения в соответствии с таблицей 2.
   ...
   |4444444444444441|	y+z|	х+5|	Ivan Ivanov|	000|
   |4444444444444441|	y|	х|	GTYUTVNHBGJN JUHBUJUI|	123|
   |4444444444444441|	y+z-1|	х+5|	Ivan Ivanov|	123|
   |444444444444441|	y|	х+1	|TO  | TE|	000|
   |4444444444444441|	01|	х+1|	IA T|	123|
   |4444444444444441|	y|	х|	Ivan Ivanov|	123|
   |4444444444444441|	y+z|	х|	GTYUTVNHBGJNM JUHBUJUI|	123|
   |4444444444444441|	01|	х+1|	kolya rik|	000|
   |4444444444444441|	y+z|	х+5|	IA T|	123|
   |4444444444444441|	01|	х+1|	TO   TE|	123|
   |4444444444444441|	y+z-1|	х+1|	GTYUTVNHBGJNM JUHBUJUI|	000|
   |4444444444444441|	y	|х+5|	IA T|	000|
   |4444444444444441|	y+z-1|	х+5|	GTYUTVNHBGJN JUHBUJUI|	000|
   |4444444444444441|	y+z|	х|	TO   TE|	000|
   |4444444444444441|	y|	х+5|	GTYUTVNHBGJNM JUHBUJUI|	123|
   ...
   |4444444444444441|	y+z|	х+1|	kolya rik|	123|
   |4444444444444441|	y+z-1|	х+5|	TO   TE|	123|
   |4444444444444441|	y+z-1|	х	|kolya rik	|123|
   |4444444444444441|	y+z|	х+5|	GTYUTVNHBGJN JUHBUJUI|	123|
...
4. Ожидаемый результат - кнопка "Продолжить" нажимается, форма отправляется, сообщение от банка
   ...
   (Владелец, Номер карты, Месяц, Год, CVC/CVV), статус Approve. Запись в СУБД выполнена.
...
4. Ожидаемый результат - кнопка "Продолжить" нажимается, форма не отправляется, в поле значение 4444 44444 4444 444 (0000 0000 0000 0000),
...
4. Ожидаемый результат - кнопка "Продолжить" нажимается, форма не отправляется, на экране под
   полем появляется сообщение "Неверно указан срок действия карты". 
...
4. Ожидаемый результат - кнопка "Продолжить" нажимается, форма не отправляется, поле не заполнено,
   на экране под полем появляется сообщение "Неверный формат". 
...
3. Остальные поля формы заполнить валидными значениями, кликнуть по кнопке "Продолжить";
4. Ожидаемый результат - кнопка "Продолжить" нажимается, форма не отправляется, поле пустое, на экране
   под полем появляется сообщение "Неверный формат". 
...
3. Остальные поля формы заполнить валидными значениями, кликнуть по кнопке "Продолжить";
4. Ожидаемый результат - в поле внесено значение без символов, кнопка "Продолжить" нажимается, форма не отправляется,
   на экране под полем появляется сообщение "Поле обязательно для заполнения". 
...
_Сценарий 2.4 Тестирование отправки формы_
...
_Сценарий 2.4.3 Тестирование отправки формы. Негативное_
1. Выполнить сценарии 1.2 или 1.4;
2. Внести валидные значения в соответствии с таблицей.

   |cardNumber  |  month | year| owner |CVC/CVV|
            |-------|:---:|-----:|-----:|-----:|
   |4444 4444 4444 4443|	y|	х+1	|NIK DIK|     123|

3. Ожидаемый результат - кнопка "Продолжить" нажимается, форма отправляется, сообщение от банка
   "Ошибка Ошибка! Банк отказал в проведении операции." В ответе от симулятора - внесены данные по операции
   (Владелец, Номер карты, Месяц, Год, CVC/CVV), статус DECLINED. Выполнена запись в СУБД не выполнена. Статус от сервера: 500.
